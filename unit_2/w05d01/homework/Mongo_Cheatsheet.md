...
To Run Mongo Server:
**In Terminal:**
- Run `brew services` to see if mongodb is running
- If it's stopped, run `brew services start mongodb-community@6.0` to start the database

***

To use the mongo shell in terminal

**In Terminal - new tab must run with `mongosh`**
- After starting `mongod` in another tab open a new tab and type
- `mongosh`
...
- You can also leave this service running in the background once you've launched it -- you don't need to stop the mongo services unless your computer is overworked.
